...
# Optional: only activates if you drop your own Firebase service-account
# JSON (from https://console.firebase.google.com -> Project settings ->
# Service accounts -> Generate new private key) at the path below.
# The app works perfectly fine without it (chatbot logging just gets skipped).
firestore_db = None
...
if os.path.exists(FIREBASE_CRED_PATH):
    try:
        ...
        print(f"Firebase not initialized (running without it): {e}")
else:
    print("No firebase-credentials.json found - running without Firebase (optional feature).")
...
# ---------------- Groq (fast, free-tier LLM) setup for the multilingual chatbot ----------------
...
if GROQ_API_KEY:
    try:
        ...
        print(f"Groq not configured (chatbot will show a setup message): {e}")
else:
    print("GROQ_API_KEY not set in .env - chatbot will show a setup message.")
...
crop_recommendation_model_path = os.path.join(BASE_DIR, 'model', 'RandomForest.pkl')
crop_recommendation_model = pickle.load(
    open(crop_recommendation_model_path, 'rb'))


soil_type_prediction_model_path = os.path.join(BASE_DIR, 'model', 'DenseNet121v2_95.h5')
...
model_path = os.path.join(BASE_DIR, 'model', 'SoilNet_93_86.h5')
...
def soil_prediction():
    title = 'FarmVision - Soil Prediction'
...
def disease_prediction():
    title = 'FarmVision - Disease Detection'
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            if prediction:
                ...
                # Real location from the farmer's browser (see disease.html geolocation script).
                # Falls back to Mumbai's coordinates only if location wasn't shared/available.
                try:
                    latitude = float(request.form.get('latitude') or 19.0760)
                    longitude = float(request.form.get('longitude') or 72.8777)
...
# ---------------- Farmers directory (real, searches the User table) ----------------
@app.route('/farmers')
def farmers_search():
    ...
    return render_template('farmers.html', title='FarmVision - Farmers', results=results, query=query)


# ---------------- Weather (real, uses OPEN_WEATHER_APIKEY + optional geolocation) ----------------
@app.route('/weather')
def weather_page():
    ...
    try:
        if lat and lon:
            ...
            if data:
                weather = {'temperature': data['temperature'], 'humidity': data['humidity']}
                city = city or 'Your location'
            else:
                error = "Couldn't fetch weather for your location right now."
        elif city:
            ...
            if result:
                ...
                error = f"Couldn't find weather for '{city}'. Check the spelling and try again."
    except Exception as e:
        print(f"Weather lookup failed: {e}")
        error = "Couldn't fetch weather right now. Please try again."

    return render_template('weather.html', title='FarmVision - Weather', weather=weather, city=city, error=error)


# ---------------- Yearly crop planning calendar (static reference content) ----------------
@app.route('/yearly-plan')
def yearly_plan():
    seasons = [
        {
            'name': 'Kharif (June - October)', 'icon': '🌧️',
            'crops': ['Rice', 'Maize', 'Cotton', 'Soybean', 'Groundnut', 'Pigeon Pea'],
            'note': 'Sown with the onset of monsoon; needs good rainfall or irrigation.',
        },
        {
            'name': 'Rabi (October - March)', 'icon': '❄️',
            'crops': ['Wheat', 'Mustard', 'Chickpea', 'Barley', 'Lentil', 'Peas'],
            'note': 'Sown after monsoon retreats; grows through the cool winter months.',
        },
        {
            'name': 'Zaid (March - June)', 'icon': '☀️',
            'crops': ['Watermelon', 'Muskmelon', 'Cucumber', 'Moong', 'Vegetables'],
            'note': 'Short summer season between Rabi and Kharif; needs irrigation.',
        },
    ]
    return render_template('yearly_plan.html', title='FarmVision - Yearly Plan', seasons=seasons)


# ---------------- Community knowledge sharing (real, backed by the database) ----------------
@app.route('/community', methods=['GET', 'POST'])
def community():
    if request.method == 'POST':
        author_name = (request.form.get('author_name') or 'Anonymous Farmer').strip()
        crop = (request.form.get('crop') or '').strip()
        tip_text = (request.form.get('tip') or '').strip()
        if tip_text:
            db.session.add(CommunityTip(author_name=author_name, crop=crop, tip=tip_text))
            db.session.commit()
            flash('Thanks for sharing! Your tip is now live.', 'success')
        return redirect(url_for('community'))

    tips = CommunityTip.query.order_by(CommunityTip.id.desc()).all()
    return render_template('community.html', title='FarmVision - Community', tips=tips)


# ---------------- Multilingual farmer chatbot (Groq / Llama 3.3) ----------------
CHATBOT_SYSTEM_PROMPT = (
    "You are FarmVision's assistant, helping Indian farmers with crop diseases, "
    "fertilizer choices, crop selection and general farming questions. "
    "Always reply in the SAME language the farmer used (Hindi, Hinglish, Marathi, "
    "Tamil, Telugu, Bengali, Punjabi, English, etc). Keep answers short, practical, "
    "and easy for a farmer to act on. Avoid jargon; explain simply."
)


@app.route('/chatbot')
def chatbot_page():
    title = 'FarmVision - Farmer Assistant'
    return render_template('chatbot.html', title=title, chatbot_ready=bool(groq_client))
...
def chatbot_ask():
    ...
    if not groq_client:
        return jsonify({
            'reply': ("Chatbot is not set up yet. Add a free GROQ_API_KEY in your .env file "
                      "(get one at https://console.groq.com/keys) and restart the app.")
        }), 200
    ...
    try:
        ...
        # Optional: log the conversation to Firestore if it's configured.
        if firestore_db:
            try:
                firestore_db.collection('chatbot_logs').add({
                    'question': user_message,
                    'answer': reply_text,
                    'timestamp': firestore.SERVER_TIMESTAMP,
                })
            except Exception as log_err:
                print(f"Firestore logging skipped: {log_err}")

        return jsonify({'reply': reply_text})
    except Exception as e:
        print(f"Groq error: {e}")
        return jsonify({'reply': 'Sorry, something went wrong answering that. Please try again.'}), 200
